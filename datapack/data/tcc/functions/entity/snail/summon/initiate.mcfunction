...
tp @s ~ ~ ~
execute unless data storage tcc:storage root.temp.snail_bucket if predicate tcc:random_chance/0.5 run tp @s ~0.1 ~ ~
execute store result entity @s Rotation[0] float 0.1 run random value -1800..1800
data remove entity @s Offers.Recipes
execute if data storage tcc:storage root.temp.snail_bucket.tag.tcc.variant run return run function tcc:entity/snail/summon/set_variant_from_bucket_macro with storage tcc:storage root.temp.snail_bucket.tag.tcc
