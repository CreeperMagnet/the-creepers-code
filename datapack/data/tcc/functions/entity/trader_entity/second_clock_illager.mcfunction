...
execute if entity @s[tag=tcc.enchanter,tag=!tcc.enchanter.casting,predicate=tcc:chance/one_fifth] if entity @e[team=tcc.antivillager,distance=..10,tag=!tcc.enchanter,tag=!tcc.snail,predicate=tcc:global.ignore,nbt=!{ActiveEffects:[{Ambient:1b}]}] run function tcc:entity/enchanter/spell_start
