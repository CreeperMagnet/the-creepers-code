...
execute unless entity @s[scores={tcc.dummy2=..2}] run tellraw @s {"fallback": "You are using The Creeper's Code on the incorrect Minecraft version. You should be using Minecraft 1.20.","translate":"commands.tcc.wrong_version","color":"red","bold":true}
execute unless entity @s[scores={tcc.dummy2=..2}] run title @s title {"translate":"commands.tcc.initiate.wrong_version.title","color":"red","bold":true,"fallback": "WRONG VERSION"}
