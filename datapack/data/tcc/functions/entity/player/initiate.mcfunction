...
scoreboard players set @s tcc.dummy 2728
...
execute unless entity @s[scores={tcc.dummy2=-4..0}] run tellraw @s {"translate":"commands.tcc.wrong_version: You are using TCC on the incorrect Minecraft version. Please check the download.","color":"red","bold":true}
execute unless entity @s[scores={tcc.dummy2=-4..0}] run title @s[tag=!global.ignore.gui] title {"translate":"commands.tcc.initiate.wrong_version.title","color":"red","bold":true}
