...
attribute @s minecraft:generic.movement_speed modifier add 0-2-0-3-3 slowness -100 add_value
execute if entity @s[tag=!tcc.has_line_of_sight] run function tcc:entity/geomancer/spell_finish
execute if entity @s[tag=tcc.has_line_of_sight] facing entity @e[predicate=tcc:entity_properties/targeted_by_illagers,limit=1,distance=..16,sort=nearest] eyes run tp @s ~ ~ ~ ~ ~
