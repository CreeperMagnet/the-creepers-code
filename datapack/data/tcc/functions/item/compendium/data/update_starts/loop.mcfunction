...
execute unless score #temp_offset tcc.dummy matches 0 store result score #temp_0 tcc.dummy run data get storage tcc:temp root.starts[-1].start
...
execute unless score #temp_offset tcc.dummy matches 0 store result storage tcc:temp root.starts[-1].start int 1 run scoreboard players get #temp_0 tcc.dummy

# Get any offsets to use
scoreboard players set #temp_0 tcc.dummy 0
execute store result score #temp_0 tcc.dummy run data get storage tcc:temp root.starts[-1].temp_offset
...
execute unless data storage tcc:temp root.starts[-1].temp_offset run data modify storage tcc:temp root.starts prepend from storage tcc:temp root.starts[-1]
data remove storage tcc:temp root.starts[-1]
