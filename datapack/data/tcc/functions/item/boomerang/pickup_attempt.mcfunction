...
execute as @a[distance=..2,sort=arbitrary,gamemode=!spectator] store result score @s tcc.dummy run data get entity @s UUID[0]
scoreboard players operation @a[distance=..2,sort=arbitrary,gamemode=!spectator] tcc.dummy -= tcc.temp_0 tcc.dummy
