...
execute if block ^ ^ ^0.05 #tcc:boomerang_ignore unless entity @e[type=shulker,nbt={Peek:0b},dx=0,dz=0,dy=0,limit=1,sort=nearest,tag=!global.ignore] run tp @s ^ ^ ^0.05
...
execute if entity @s[scores={tcc.dummy2=1..}] positioned ~-0.5 ~-0.5 ~-0.5 as @e[type=!#tcc:boomerang_ignore,dx=0,dz=0,dy=0,distance=..5,predicate=tcc:global.ignore,predicate=tcc:boomerang_damage_valid] run function tcc:item/boomerang/hurt/entity_damage
