...
execute if entity @s[advancements={tcc:technical/player_interacted_with_entity/phantom_item_frame={add_item=true}}] as @e[type=minecraft:item_frame,limit=1,sort=nearest,nbt={Item:{}},tag=!tcc.phantom_item_frame.has_item] at @s run function tcc:block/phantom_item_frame/interact/add_item_as_entity
