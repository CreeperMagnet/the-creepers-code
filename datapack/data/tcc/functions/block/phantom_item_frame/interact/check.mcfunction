...
execute if entity @s[advancements={tcc:technical/block/phantom_item_frame_add_item={rotate_empty=true}}] as @e[type=minecraft:item_frame,tag=tcc.phantom_item_frame,tag=tcc.phantom_item_frame.empty,nbt={ItemRotation:1b}] run data modify entity @s ItemRotation set value 0b
