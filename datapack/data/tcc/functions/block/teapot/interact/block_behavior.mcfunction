...
execute store success score @s tcc.dummy if entity @s[scores={tcc.dummy=0},tag=tcc.teapot.filled_tea,tag=!tcc.teapot.filled_snail_mucus,tag=!tcc.teapot.filled_warped_wart] if data storage tcc:storage root.temp.item{tag:{tcc:{id:"snail_mucus"}}} run function tcc:block/teapot/interact/set_states/add_snail_mucus
