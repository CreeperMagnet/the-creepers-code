...
execute unless score @s tcc.dummy2 matches 1.. if entity @s[tag=tcc.teapot.filled] unless data storage tcc:storage root.temp.full_inventory if data storage tcc:storage root.temp.item{id:"minecraft:glass_bottle"} run function tcc:block/teapot/interact/take_liquid/main
