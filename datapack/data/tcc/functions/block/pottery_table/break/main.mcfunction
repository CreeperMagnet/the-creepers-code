...
execute if data entity @s item.tag.stored_barrel_data[1][{Slot:15b}].id as @e[type=minecraft:item,sort=nearest,distance=..2,tag=!smithed.entity] run function tcc:block/pottery_table/break/delete_output

# Spawn particles, items, etc.
execute as @e[tag=!smithed.entity,limit=1,type=minecraft:item,distance=..3,nbt={PickupDelay:10s,Item:{id:"minecraft:barrel",tag:{display:{Name:'{"translate":"block.tcc.pottery_table.gui","with":[{"translate":"block.tcc.pottery_table","font":"minecraft:default","color":"#3F3F3F"}],"font":"tcc:pottery_table","color":"white"}'}}}}] at @s run function tcc:block/pottery_table/break/kill_item
