...
execute as @e[tag=!smithed.entity,limit=1,type=minecraft:item,distance=..3,nbt={PickupDelay:10s,Item:{components:{"minecraft:custom_name":'{"color":"white","font":"tcc:pottery_table","translate":"block.tcc.pottery_table.gui","with":[{"color":"#3F3F3F","font":"minecraft:default","translate":"block.tcc.pottery_table"}]}'},id:"minecraft:barrel"}}] at @s run function tcc:block/pottery_table/break/kill_item
