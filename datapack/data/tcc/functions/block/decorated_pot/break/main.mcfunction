...
execute as @e[tag=!smithed.entity,limit=1,type=minecraft:item,distance=..5,sort=nearest,nbt={Item:{components:{"minecraft:pot_decorations":["minecraft:air","minecraft:air","minecraft:air","minecraft:air"]},id:"minecraft:decorated_pot"}}] unless entity @s[nbt=!{Age:0s},nbt=!{Age:1s}] run function tcc:block/decorated_pot/break/as_item
