...
execute as @e[distance=..7,tag=!tcc.geomancer,tag=!smithed.strict] run damage @s 6.0 tcc:geomancer_explosive by @e[type=item_display,tag=tcc.geomancer_explosive_damage_source,limit=1] from @e[type=wandering_trader,tag=tcc.geomancer,scores={tcc.dummy=0},limit=1,sort=nearest]
