...
execute if score #soul_conductor_limit_count tcc.dummy matches 0 run title @p[advancements={tcc:technical/item_used_on_block/activate_soul_conductor=true}] actionbar {"translate":"block.tcc.soul_conductor.exceeded_path_limit","color":"red","with":[{"score":{"name":"#soul_conductor_limit","objective":"tcc.dummy"}}]}