...
## Nether Reactor
execute if entity @s[type=minecraft:item_display,tag=tcc.nether_reactor_core] if loaded ~1 ~ ~1 if loaded ~1 ~ ~-1 if loaded ~-1 ~ ~1 if loaded ~-1 ~ ~-1 run function tcc:block/nether_reactor_core/second_clock
