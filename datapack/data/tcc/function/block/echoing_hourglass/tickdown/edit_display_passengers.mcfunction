...
execute on passengers if entity @s[tag=tcc.echoing_hourglass.powder_top] run data modify entity @s transformation.scale[1] set from storage tcc:temp root.scale
execute store result storage tcc:temp root.scale float 0.025 run scoreboard players get #temp_0 tcc.dummy
execute on passengers if entity @s[tag=tcc.echoing_hourglass.powder_bottom] run data modify entity @s transformation.scale[1] set from storage tcc:temp root.scale