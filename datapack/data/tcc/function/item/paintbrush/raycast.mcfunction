...
execute positioned ~-0.01 ~-0.99 ~-0.01 as @e[dx=0,type=!#tcc:intangible,tag=!tcc.paintbrush.raycaster] positioned ~-0.99 ~0.99 ~-0.99 if entity @s[dx=0] run return run execute if entity @s[type=#tcc:paintbrush_affected] run function tcc:item/paintbrush/as_entity
