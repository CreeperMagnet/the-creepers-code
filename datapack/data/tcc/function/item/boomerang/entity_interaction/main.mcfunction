...
$execute if entity @s[type=!minecraft:enderman,nbt=!{Invulnerable:1b}] unless predicate {"condition":"minecraft:entity_properties","entity":"this","predicate":{"passenger":{"nbt":"{UUID:$(UUID)}"}}} run damage @s 4.0 tcc:boomerang by @p[nbt={UUID:$(UUID)}]
