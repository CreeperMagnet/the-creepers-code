...
execute if data storage tcc:temp root.item{slot:"weapon.offhand"} unless data entity @s[gamemode=!creative] equipment.offhand run return run function tcc:technical/macros/loot/replace with storage tcc:temp root.item
execute if data storage tcc:temp root.item{slot:"weapon.offhand"} if data entity @s[gamemode=!creative] equipment.offhand run data modify storage tcc:temp root.item.slot set value 0
